...
public class Professor extends Person {
	...
	public void input_data() {
		...
		System.out.println("input professor_number");
		Professor_number = keyboard.nextInt();
		 ...
		 */
		if(Fileio(Professor_number) == false)
		{
	...
	public boolean Fileio(int new_number)// 매개변수는 학번으로.
	{
		 ...
		 * 입력받아 정보를 임시로 변수에 저장한다 1-1. 이때 강의는 하는 강의의 개수만큼만 저장한다
		 * 
		 * 2. 입력받은 정보의 학번이 매개변수의 학번과 같다면 멤버 변수에 입력받은 정보를 저장한다. 2-2. 강의 개수가 3개 미만이면 나머지는
		 * null로 채워진다 -->예) 만약 강의를 2개 하는 교수라면 Lecture_list[2]에는 null이 저장된다.(출력시 null 아닌
		 * 값만 출력)
		 * 
		...
		try {
			...
			while (inputStream.hasNextLine()) {
				...
				int ProNumber = Integer.parseInt(data[0]);
				String name = data[1];
				String Phone = data[2];
				...
				if (Integer.compare(new_number, ProNumber) == 0) // 학번과 일치할 경우 정보를 객체의 member 변수에 저장한다
				{
					Name = name;
					phone_number = Phone;
					Lecture_list = lecture;
					
					introduce[0] = Integer.toString(Professor_number);
					introduce[1] = name;
					introduce[2] = Phone;
